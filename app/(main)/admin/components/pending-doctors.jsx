...
export function PendingDoctors({ doctors }) {
  ...
  // Hook customizado para aprovar/rejeitar médico
  const {
    loading,
    data,
  ...
  return (
    <div>
      <Card className="bg-muted/20 border-emerald-900/20">
        <CardHeader>
          <CardTitle className="text-xl font-bold text-white">
            Verificações de Médicos Pendentes
          </CardTitle>
          <CardDescription>
            Revise e aprove as solicitações de cadastro de médicos
          </CardDescription>
        </CardHeader>
        <CardContent>
          {doctors.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              Nenhuma solicitação de verificação pendente no momento.
            </div>
          ) : (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {doctor.specialty} • {doctor.experience} anos de
                            experiência
                          </p>
                        </div>
                      </div>
      ...
      {/* Modal com Detalhes do Médico */}
      {selectedDoctor && (
        <Dialog open={!!selectedDoctor} onOpenChange={handleCloseDialog}>
          <DialogContent className="max-w-3xl">
            <DialogHeader>
              <DialogTitle className="text-xl font-bold text-white">
                Detalhes da Verificação do Médico
              </DialogTitle>
              <DialogDescription>
                Revise cuidadosamente as informações do médico antes de tomar
                uma decisão
              </DialogDescription>
            </DialogHeader>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-6">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-white">
                      {selectedDoctor.experience} anos
                    </p>
                  </div>
