  ...
  return (
    <>
      <div className="flex flex-col items-center justify-center mb-8 text-center">
        <h1 className="text-3xl font-bold text-white mb-2">Encontre seu médico</h1>
        <p className="text-muted-foreground text-lg">
        Navegue por especialidade ou veja todos os profissionais de saúde disponíveis
        </p>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {SPECIALTIES.map((specialty) => (
          <Link prefetch key={specialty.name} href={`/doctors/${specialty.name}`}>
